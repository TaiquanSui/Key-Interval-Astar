#include "KeyIntervalAStarOptimal.h"
#include <algorithm>
#include <chrono>
#include <windows.h>
...
KeyIntervalAStarOptimal::KeyIntervalAStarOptimal(const Preprocess &preprocess, const std::string &name)
...
KeyIntervalAStarOptimal::KeyIntervalAStarOptimal(const std::string &name)
    : preprocess_(nullptr), name_(name)
{
}

// new: preprocessing method
void KeyIntervalAStarOptimal::preprocess(const std::vector<std::vector<int>>& grid) {
...
std::vector<Vertex> KeyIntervalAStarOptimal::search(const Vertex &start, const Vertex &target)
{
    ...
    std::unordered_map<StateKey, double, StateKeyHash> gScore; // record g value with StateKey
...
KeyIntervalAStarOptimal::KeyIntervalQueryResult KeyIntervalAStarOptimal::queryKeyIntervals(const Vertex &start, const Vertex &target) const
{
...
void KeyIntervalAStarOptimal::initializeStartNode(const Vertex &start, const Vertex &target,
...
KeyIntervalAStarOptimal::SearchNode KeyIntervalAStarOptimal::handleNeighbor(const KeyIntervalAStarOptimal::SearchNode &current,
...
KeyIntervalAStarOptimal::SearchNode KeyIntervalAStarOptimal::directTransition(const KeyIntervalAStarOptimal::SearchNode &current,
                                                                const NeighborTriple &neighborTriple,
                                                                ...
                                                                const Vertex &target)
{
    ...
    if (checkAndAddUpDownVertex(transitionVertex, current.waypoints.back(), newWaypoints, current))
    {
        ...
        KeyIntervalAStarOptimal::SearchNode newNode = KeyIntervalAStarOptimal::SearchNode(neighborTriple.neighborKeyInterval, g, g + h,
                                                                            current.keyInterval,
...
KeyIntervalAStarOptimal::SearchNode KeyIntervalAStarOptimal::handleUpDownVertex(const KeyIntervalAStarOptimal::SearchNode &current,
                                                                  const NeighborTriple &neighborTriple,
                                                                  const Vertex &target)
{
...
bool KeyIntervalAStarOptimal::handleTargetKeyIntervals(const SearchNode &current, const Vertex &target,
                                                const KeyIntervalQueryResult &queryResult,
                                                std::priority_queue<SearchNode, std::vector<SearchNode>, std::greater<SearchNode>> &openList,
                                                std::unordered_map<StateKey, double, StateKeyHash> &gScore)
{
...
KeyIntervalAStarOptimal::SearchNode KeyIntervalAStarOptimal::handleTargetKeyInterval(const KeyIntervalAStarOptimal::SearchNode &current,
                                                                       ...
                                                                       const Vertex &target)
{
    ...
    if (transitionVertexOpt.has_value())
    {
        ...
        if (checkAndAddUpDownVertex(transitionVertex, current.waypoints.back(), finalWaypoints, current))
        {
            ...
            KeyIntervalAStarOptimal::SearchNode targetNode = KeyIntervalAStarOptimal::SearchNode(std::nullopt, g, g,
                                                                                   current.keyInterval,
                                                                                   std::nullopt,
    ...
    KeyIntervalAStarOptimal::SearchNode targetNode(std::nullopt, g, g, current.keyInterval, std::nullopt,
                                            std::nullopt, std::nullopt, finalWaypoints, true);
    //logger::log_info("push target node");
    return targetNode;
}

std::pair<double, double> KeyIntervalAStarOptimal::calcGAndH(const std::vector<Vertex> &waypoints,
...
std::vector<Vertex> KeyIntervalAStarOptimal::constructPath(std::vector<Vertex> waypoints) const
{
...
void KeyIntervalAStarOptimal::insertNode(const SearchNode &node,
                                  std::priority_queue<SearchNode, std::vector<SearchNode>, std::greater<SearchNode>> &openList,
                                  std::unordered_map<StateKey, double, StateKeyHash> &gScore)
{
    ...
    // check if g value is better using StateKey
    StateKey skey{node.keyInterval.value(), node.waypoints.back(), node.upVertex, node.downVertex};
    if (gScore.find(skey) == gScore.end() || node.g < gScore[skey])
    {
        gScore[skey] = node.g;
        openList.push(node);
...
bool KeyIntervalAStarOptimal::checkAndAddUpDownVertex(const Vertex &targetVertex,
